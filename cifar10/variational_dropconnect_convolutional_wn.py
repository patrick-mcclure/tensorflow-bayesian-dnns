...
class _Conv(base.Layer):
  ...
  def __init__(self, rank,
    ...
    self.a_initializer = tf.constant_initializer(math.log(0.01))#tf.random_normal_initializer(-4.5,0.01)
  ...
  def build(self, input_shape):
    ...
    self.kernel_sigma = tf.multiply(tf.exp(self.kernel_a),self.kernel_m,name = 'kernel_sigma')
    
    tf.add_to_collection('sigmas',self.kernel_sigma)
    tf.summary.scalar(self.kernel_sigma.name, tf.reduce_mean(self.kernel_sigma))
    self.kernel_e = tf.random_normal(self.kernel_m.shape)
    ...
    if self.use_bias:
      ...
      self.bias_a = self.add_variable(name='bias_a',
                                    shape=(self.filters,),
                                    initializer=self.a_initializer,
                                    regularizer=None,
                                    constraint=None,
      ...
      self.bias_sigma = tf.multiply(tf.exp(self.bias_a),self.bias_m, name='bias_sigma')
      tf.add_to_collection('sigmas',self.bias_sigma)
      tf.summary.scalar(self.bias_sigma.name, tf.reduce_mean(self.bias_sigma))
      self.bias_e = tf.random_normal(self.bias_m.shape)
