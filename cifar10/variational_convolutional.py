...
class _Conv(base.Layer):
  ...
  def __init__(self, rank,
    ...
    self.log_sigma_initializer = tf.constant_initializer(-4.5)
  ...
  def build(self, input_shape):
    ...
    #self.v_norm = tf.nn.l2_normalize(self.v,[0,1,2,3])
    
    #self.kernel_m = tf.multiply(self.g,self.v_norm,name = 'kernel_m')

    self.kernel_m = self.add_variable(name='kernel_m',
                                    shape=kernel_shape,
                                    initializer=self.kernel_initializer,
    ...
    self.kernel_s = self.add_variable(name='kernel_s',
                                    shape=kernel_shape,
                                    initializer=self.log_sigma_initializer,
                                    regularizer=None,
                                    constraint=None,
                                    trainable=True,
                                    dtype=self.dtype)
    
    self.kernel_sigma = tf.exp(self.kernel_s,name = 'kernel_sigma')
    
    self.kernel_e = tf.random_normal(self.kernel_s.shape)

    self.kernel = tf.add(self.kernel_m,self.kernel_sigma*self.kernel_e,name='kernel')

    if self.use_bias:
      self.bias_m = self.add_variable(name='bias_m',
                                    shape=(self.filters,),
      ...
      self.bias_s = self.add_variable(name='bias_s',
                                    shape=(self.filters,),
                                    initializer=self.log_sigma_initializer,
                                    regularizer=None,
                                    constraint=None,
      ...
      self.bias_sigma = tf.exp(self.bias_s, name='bias_sigma')
      
      self.bias_e = tf.random_normal(self.bias_s.shape)
      
      self.bias = tf.add(self.bias_m,self.bias_sigma*self.bias_e,name='bias')
